...
DEFAULT_EXTERNAL = ROOT / "baselines"
DEFAULT_SHARED_DATA = ROOT / "data" / "derived" / "m-owodb-voc"
DEFAULT_RESULTS = ROOT / "exps" / "owod" / "m-owodb" / "order0" / "official" / "baselines"
METHODS = {
    "prob": {
...
def command_for(record, gpus, data_root, results_root):
    ...
    extra = "" if config.endswith("_SHARED.sh") else " --dataset TOWOD --data_root ./data/OWOD"
    return (f"cd {record['path']} && "
            f"MOWODB_DATA_ROOT={shell_path(data_root)} "
            f"MOWODB_OUTPUT_ROOT={shell_path(results_root)} "
            f"CUDA_VISIBLE_DEVICES={gpus} GPUS_PER_NODE={count} "
            f"./tools/run_dist_launch.sh {count} {config}{extra}")


def stage_commands(record, gpus, data_root, results_root):
    ...
    root = record["path"]
    count = len([item for item in gpus.split(',') if item.strip()])
    prefix = (f"cd {root} && MOWODB_DATA_ROOT={shell_path(data_root)} "
              f"MOWODB_OUTPUT_ROOT={shell_path(results_root)} "
              f"CUDA_VISIBLE_DEVICES={gpus} GPUS_PER_NODE={count}")
    ...
    train_extra = "" if train_config.endswith("_SHARED.sh") else f" {suffix}"
    eval_extra = "" if eval_config.endswith("_SHARED.sh") else f" {suffix}"
    train = f"{prefix} ./tools/run_dist_launch.sh {count} {train_config}{train_extra}"
    evaluate = f"{prefix} ./tools/run_dist_launch.sh {count} {eval_config}{eval_extra}"
...
def main(argv=None):
    ...
    parser.add_argument(
        "--allow-unverified",
        action="store_true",
        help="Label a shared local/pilot run as non-paper-comparable",
    )
    parser.add_argument("--data-root", type=Path, default=DEFAULT_SHARED_DATA,
                        help="Shared VOC data view generated by prepare_shared_mowodb.py")
    parser.add_argument("--results-root", type=Path, default=DEFAULT_RESULTS,
                        help="Root exps directory for external baseline checkpoints and logs")
    ...
    if args.allow_unverified and not args.shared_mowodb:
        parser.error("--allow-unverified requires --shared-mowodb")
    records = []
    ...
    paper_comparable = bool(
        ...
        and all(r["status"] == "ready" for r in records)
    )
    payload = {
        "schema": 1,
        "paper_comparable": paper_comparable,
        "validation_mode": ("unverified_local" if args.allow_unverified else
                             "official" if args.shared_mowodb else "external_recipe"),
        "protocol": (("shared_m-owodb_v1; local unverified; external author evaluator"
                      if args.allow_unverified else
                      "shared_m-owodb_v1; external author evaluator")
                     if args.shared_mowodb else
                     "official_external_recipe; not Tree-DETR COCO matched"),
        "records": records,
        "commands": {r["method"]: command_for(
            r, args.gpus, args.data_root, args.results_root) for r in records},
        "stage_commands": {r["method"]: stage_commands(
            r, args.gpus, args.data_root, args.results_root) for r in records},
    ...
    if args.allow_unverified:
        payload["limitations"].append(
            "The shared data view reuses a local/pilot manifest; results must not be reported as official M-OWODB scores."
        )
