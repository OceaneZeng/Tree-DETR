...
SOURCE_PLACEHOLDERS = ("<", ">", "replace", "placeholder", "todo", "替换")
...
def validate_source_reference(source_reference: str) -> str:
    ...
    if any(token in lowered_reference for token in SOURCE_PLACEHOLDERS):
        raise ValueError(
            "source_reference is still a placeholder; provide the real official "
            "release URL, repository commit, or archive ID")
    return reference


def reject_retired_local_split(annotation_root: Path) -> None:
    legacy_manifest_path = annotation_root / "split_manifest.json"
    if not legacy_manifest_path.is_file():
        return
    legacy = read_json(legacy_manifest_path)
    legacy_fields = {"grouping_source", "memory_fraction", "order", "seed"}
    if legacy.get("schema_version") == 1 and legacy_fields.issubset(legacy):
        raise ValueError(
            "annotation-root contains a schema_version=1 split generated by the "
            "retired local OWOD builder; copy genuine official annotations to a "
