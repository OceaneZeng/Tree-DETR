...
2026-09-07 执行范围更新：按用户最新要求，当前队列改为 D4 完成后运行 OW-DETR 与 EW-DETR 的四阶段受控实验。实现、来源、论文未明确的细节及服务器命令见 [paper-baselines.md](paper-baselines.md)。下面的 PROB 优先顺序保留为历史方案，不再代表当前执行队列；当前适配版不能宣称精确复现作者论文分数。
