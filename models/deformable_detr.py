...
class SetCriterion(nn.Module):
    ...
    def loss_cardinality(self, outputs, targets, indices, num_boxes):
        ...
        # Focal-loss Deformable DETR has no explicit no-object logit. The old
        # argmax-based calculation treated the final foreground class as
        # background and made this diagnostic almost equal to num_queries.
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        topk_unknown = torch.gather(unknown, 1, topk_boxes)
        results = []
        for s, l, b, u in zip(scores, labels, boxes, topk_unknown):
            results.append({
                'scores': s,
                'labels': l,
                'boxes': b,
                'unknown_scores': u,
                'unknown_mask': u >= self.unknown_threshold,
            })
