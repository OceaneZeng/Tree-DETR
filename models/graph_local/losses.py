...
def local_margin_loss(outputs: Dict[str, torch.Tensor], targets: Sequence[Dict],
    ...
    """Separate each matched local target from its strongest local competitor."""
    logits = outputs["pred_logits"]
    local = sorted({int(class_id) for class_id in local_classes})
    if len(local) < 2:
        return logits.sum() * 0.0
    if local[-1] >= logits.shape[-1]:
    ...
    indices = matcher(
        {"pred_logits": logits, "pred_boxes": outputs["pred_boxes"]},
        targets,
    )
    terms = []
    for batch_index, (source_indices, target_indices) in enumerate(indices):
        ...
        selected = [offset for offset, label in enumerate(labels.tolist()) if label in positions]
        if not selected:
            continue
        selected_tensor = torch.as_tensor(selected, device=labels.device, dtype=torch.long)
...
def projection_loss(delta_vector: torch.Tensor, off_basis: torch.Tensor) -> torch.Tensor:
    """Penalize first-order overlap with the off-neighborhood gradient basis."""
    if off_basis.ndim != 2:
    ...
    overlaps = basis.t().matmul(delta_vector)
    return overlaps.square().mean()
